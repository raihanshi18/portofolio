...
const Navbar = () => {
    ...
    const handleSetActive = (id: string) => {
        // Kalau item yang sama, skip
        if (id === activeItem) return

        if (activeItem && activeItem !== id) {
            // Ada konten aktif → fade out dulu, lalu ganti
            ...
            // Belum ada konten → langsung tampil
            setActiveItem(id)
            setVisibleItem(id)
        }
    }
    ...
    return (
        <>
            ...
            >
                ...
                {/* Bottom info bar */}
                <div
                    className={`absolute bottom-0 left-0 w-full px-24 py-10 transition-all duration-700 delay-200 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                        }`}
                >
            ...
            {/* ── BOTTOM NAVBAR (default state) ── */}
            <nav
                className={`fixed bottom-0 left-0 w-full px-24 py-10 mb-2 text-white z-50 font-clash transition-all duration-500 ${isOpen ? 'opacity-0 pointer-events-none translate-y-4' : 'opacity-100 translate-y-0'
                    }`}
            >
                <div>
                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="mt-6 text-md grid grid-cols-3 gap-x-20 gap-y-2">
                                <p>Student of Software Engineering</p>
                                <p>Bandung, West Java</p>
                                <p
